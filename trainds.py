...
"""
Train a YOLOv5 Det & Seg model on a custom dataset
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg, segcfg, resume, noval, nosave, workers, freeze, = \
        Path(
            opt.save_dir), opt.epochs, opt.batch_size, opt.weights, opt.single_cls, opt.evolve, opt.data, opt.cfg, opt.segcfg, \
    ...
    # Process 0
    if RANK in [-1, 0]:
        val_loader = create_dataloader(val_path, nc, imgsz, batch_size // WORLD_SIZE * 2, gs, single_cls,
    ...
    # Model attributes
    # nl = de_parallel(model).model[24].nl  # number of detection layers (to scale hyps)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        sloss = torch.zeros(1, device=device)  # segmentation mean loss
        if RANK != -1:
            train_loader.sampler.set_epoch(epoch)
        pbar = enumerate(train_loader)
        segpbar = enumerate(roadseg_train_loader)
        ############################ detection ######################
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                ...
                # print("pred:",type(pred),len(pred),type(pred[0]), [p.shape for p in pred[0]])
        ...
        ########################
        # with open("yoloprelayerloss.txt","a+") as fl:
        #     fl.writelines("Epoch:{} || YOLO Loss : {}\n".format(epoch,str(compute_loss.loss_perlayer.detach().cpu().numpy().tolist())))
        # print("loss_perlayer:",compute_loss.loss_perlayer)
        if RANK in [-1, 0]:
            # mAP
