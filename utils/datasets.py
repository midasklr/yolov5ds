...
def create_dataloader(path, clsnum, imgsz, batch_size, stride, single_cls=False, hyp=None, augment=False, cache=False,
                      pad=0.0,
    ...
    with torch_distributed_zero_first(rank):  # init dataset *.cache only once if DDP
        dataset = LoadImagesAndLabels(path, clsnum, imgsz, batch_size,
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self, path, cls_num=80, img_size=640, batch_size=16, augment=False, hyp=None, rect=False,
        ...
        self.counter_per_cls = [0] * cls_num
        for label in labels:
            labelnum = [label[it, 0] for it in range(len(label))]
            for it in labelnum:
                self.counter_per_cls[int(it)] += 1

        # count_balance = [1000*math.pow(it,1/5) for it in self.counter_per_cls]
        # max_num = max(count_balance)
        # self.cls_balance = [max_num/it for it in count_balance]
